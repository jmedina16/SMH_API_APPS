...
if (isset($_GET['iSortCol_0'])) {
    for ($i = 0; $i < intval($_GET['iSortingCols']); $i++) {
        if ($_GET['bSortable_' . intval($_GET['iSortCol_' . $i])] == "true") {
            $filter[0]->orderBy = ($_GET['sSortDir_' . $i] == 'asc' ? '+' : '-') . $aColumns[intval($_GET['iSortCol_' . $i])];
...
if (isset($_GET['sSearch']) && $_GET['sSearch'] != "") {
    $filter[0]->freeText = $_GET['sSearch'];
}

//mediaTypeIn
if (isset($_GET['mediaTypeIn']) && $_GET['mediaTypeIn'] != "") {
    $filter[0]->mediaTypeIn = $_GET['mediaTypeIn'];
}

// Entry 
if (isset($_GET['entry']) && $_GET['entry'] != "") {
    $filter[0]->isRoot = $_GET['entry'];
}

//Categories
if (isset($_GET['category']) && $_GET['category'] != "" && $_GET['category'] != undefined && $_GET['category'] != null && $_GET['category'] != 'null') {
    $filter[0]->categoriesIdsMatchOr = $_GET['category'];
}

$detailed = null;
$filteredListResult = $client->playlist->executeFromFilters($filter, $totalResults, $detailed);
