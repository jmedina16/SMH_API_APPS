...
function reset_pass($pid, $sm_ak, $reset_token, $email, $pass) {
    $ch = curl_init();
    curl_setopt($ch, CURLOPT_URL, "https://mediaplatform.streamingmediahosting.com/apps/mem/v1.0/index.php?action=reset_pass&pid=" . $pid . "&sm_ak=" . $sm_ak . "&reset_token=" . $reset_token . "&email=" . $email . "&pass=" . $pass);
            ...
            <div style="width: 500px; margin-left: auto; margin-right: auto; margin-top: 160px; font-size: 17px;" id="pass-wrapper">
                ...
                <form id="smh-password-form" action="">
                    <table style="width: 400px; margin-left: auto; margin-right: auto;">
                        ...
                        <tr>
                            <td><span id="reset-loading" style="margin-right: 20px; display: block; height: 20px; float: right; display: none;"><img src="/img/loading.gif" height="20px" /></span></td><td><button style="width: 112px; display: block; font-size: 15px;" type="button" class="btn btn-primary" data-dismiss="modal" onclick="resetPassword();">Submit</button></td>
                        </tr>
                    </table>  
                </form>
            </div>
        ...
        <script>   
            ...
            function resetPassword(){                
                ...
                if(valid){
                    ...
                    $.ajax({
                        type: "GET",
                        url: "https://mediaplatform.streamingmediahosting.com/apps/mem/v1.0/index.php?action=reset_pass",
                        data: sessData,
                        dataType: 'json',
                        beforeSend: function() {                    
                        ...
                        if(data['success']){
                            ...
                            $('#pass-wrapper').html('<div style="margin-top: 30px; width: 388px; margin-left: auto; margin-right: auto; text-align: center; height: 125px;">'+
                                '<h3>Invalid Key</h3>'+
                                'This key is either invalid or has already been used and is deactivated.'+
                                '</div>');
                        }
                    });
                }
