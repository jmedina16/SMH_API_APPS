...
class BsfFlavorSelector {
  ...
  public function __contruct($db_link, $ks, $payload){
  ...
  public function convertFlavors(){
    $flavors = array();
      if($this->payload['status'] == 2){
        foreach($this->payload['flavor'] as $f){
          if($f['fileType'] == 'video'){
            $flavors = $this->getVideoFlavors($f['id'],$f['height'],$f['bitrate'],$f['fileExt']);
          } else if($f['fileType'] == 'audio'){
            $flavors = $this->getAudioFlavors($f['id'],$f['bitrate'],$f['fileExt']);
          }
        }
