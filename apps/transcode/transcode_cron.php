    ...
    public function insert_transcoded_flavors() {
        ...
        foreach ($this->file_sync_entries_file_sizes as $flavors) {
            try {
                ...
                print_r($flavors);
                print_r("INSERT INTO transcoding (partner_id, entry_id, media_type, flavor, width, height, bitrate, frame_rate, version, file_size, length_in_msecs, ready_at) SELECT * FROM (SELECT " . $flavors['partner_id'] . ", '" . $flavors['entry_id'] . "', " . $flavors['media_type'] . ", '" . $flavors['flavor'] . "', '" . $flavors['width'] . "', '" . $flavors['height'] . "', '" . $flavors['bitrate'] . "', '" . $flavors['frame_rate'] . "', " . $flavors['version'] . ", " . $flavors['file_size'] . ", " . $flavors['length_in_msecs'] . ", '" . $ready_at . "') AS tmp WHERE NOT EXISTS (SELECT flavor, version FROM transcoding WHERE flavor = '" . $flavors['flavor'] . "' AND version = " . $flavors['version'] . ") LIMIT 1;");
