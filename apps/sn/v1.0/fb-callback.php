...
function store_facebook_authorization($pid, $ks, $code) {
    $ch = curl_init();
    curl_setopt($ch, CURLOPT_URL, "http://mediaplatform.streamingmediahosting.com/apps/sn/v1.0/index.php?action=store_facebook_authorization&pid=" . $pid . "&ks=" . $ks . "&code=" . $code);
    ...
    <body>
        ...
        if ((isset($_GET['state']) && !empty($_GET['state'])) && (isset($_GET['code']) && !empty($_GET['code']))) {
            ...
            <div style="margin-top: 30px; width: 450px; margin-left: auto; margin-right: auto; text-align: center; height: 125px;">
                <h3><img src="/img/facebook_logo.png" width="150px"></h3>
                Access has been denied to your Facebook account. 
            </div>            
        </div>';
        } else {
            echo '<div style="width: 500px; margin-left: auto; margin-right: auto; margin-top: 240px; font-size: 17px;" id="loading">
